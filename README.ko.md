...
| 기능 | 방법 | 성능 |
|-----------|-----|:----------:|
| **동일 역할 검색** | 쿼리와 동일한 팔괘 역할을 가진 개념 검색 | 42% P@1 (same domain), 100% R@10 |
| **상보적 발견** | 모든 개념의 대척점 발견 (GA-Bagua 고유) | 정확한 괘 수준 매칭 |
| **오행 경로 탐색** | 상생/상극 체인을 따른 멀티홉 탐색 | 홉당 500ns |
| **멀티홉 합성** | 로터 대수로 100개 추론 단계 합성 | 200µs, 제로 드리프트 |
| **인코딩 안정성** | 동일 개념 → 매번 동일 레이블 | 99.8% (±5% 노이즈 하) |
| **개념 진화** | 한 측면이 변할 때 개념의 변화 예측 | 결정론적 변효 변환 |
| **관계 분류** | LLM 검증을 위한 방향성 힌트 | 45–52% 테스트 정확도 |
| **저장 밀도** | 개념당 64바이트. 100만 개념 = 64 MB | BERT보다 48배 밀도 |
| **제로 쿼리 비용** | 인코딩 후 모든 연산은 순수 대수 | 0 토큰, 연산당 500ns |
| **선명도 게이트** | 무작위 노이즈는 신뢰도 0.0 | 93.5% 무작위 쌍 차단 |
| **문서 정렬** | 관계 분류를 통한 문서 간 주장 매칭 | Precision@5 ≥ 70% |
| **정책 일관성** | 문서 내/문서 간 모순 조항 탐지 | F1 ≥ 0.67 |
| **논증 분석** | 순환, 비논리적, 모순적 논증 탐지 | F1 ≥ 0.89 |
